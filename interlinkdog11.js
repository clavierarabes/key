...
function getLabels() {
  var labels = [];
  var labelElements = document.querySelectorAll('.label-name a');
...
function generateInterlinks(labels) {
  ...
  document.querySelectorAll('.label-name a').forEach(function (labelElement) {
...
function displayInterlinks(interlinks) {
  ...
  if (articleContent) {
    var interlinksHtml =
      '<div style="margin-top: 20px; border-top: 2px solid #ccc; padding-top: 10px;"><h3>Articles connexes :</h3><p>' +
      interlinks.join(' | ') +
      '</p></div>';
